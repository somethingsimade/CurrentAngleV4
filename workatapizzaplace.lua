...
local UI = (gethui and gethui())
    or (cloneref and cloneref(game:GetService('CoreGui')))
    or (pcall(function()
        return game:GetService('CoreGui').Parent
...
local nametoexcludefromtransparency = _G['Names to exclude from transparency']
    or {}
local parentrealchartofakechar = _G['Parent real character to fake character']
    or false
...
local hiderootpart = (function()
    if _G['Hide HumanoidRootPart'] == nil then
        return false
    else
        return _G['Hide HumanoidRootPart']
    end
end)()
local permadeathcharacter = (function()
    if _G['PermaDeath fake character'] == nil then
        return true
    else
        return _G['PermaDeath fake character']
    end
end)()
--local r15rig = (function() if _G["R15 Reanimate"] == nil then return false else return _G["R15 Reanimate"] end end)()
local clickfling = (function()
    if _G['Click Fling'] == nil then
        return false
    else
        return _G['Click Fling']
    end
end)()
...
for i, v in
    ipairs(
        game:GetService('Players').LocalPlayer.PlayerGui.MainGui.Menu.Emotions.ScrollingFrame.List
            :GetChildren()
    )
do
    if v:FindFirstChild('TextLabel') and v.TextLabel.Text == 'Trip' then
...
for i, v in
    ipairs(
        getconnections(
            game.Players.LocalPlayer.Character.Humanoid:GetPropertyChangedSignal(
                'Jump'
            )
        )
    )
...
local fakeChar = game:GetService('Players'):CreateHumanoidModelFromDescription(
...
game:GetService('RunService').Heartbeat:Connect(function()
    gameNewIndex(
        LeftUpperArm,
        'CFrame',
        CFrameMul(gameIndex(LeftArm, 'CFrame'), UpperArmOffset)
    )
    gameNewIndex(
        RightUpperArm,
        'CFrame',
        CFrameMul(gameIndex(RightArm, 'CFrame'), UpperArmOffset)
    )
    gameNewIndex(
        LeftLowerArm,
        'CFrame',
        CFrameMul(gameIndex(LeftArm, 'CFrame'), LowerArmOffset)
    )
    gameNewIndex(
        RightLowerArm,
        'CFrame',
        CFrameMul(gameIndex(RightArm, 'CFrame'), LowerArmOffset)
    )
    gameNewIndex(
        LeftHand,
        'CFrame',
        CFrameMul(gameIndex(LeftArm, 'CFrame'), HandOffset)
    )
    gameNewIndex(
        RightHand,
        'CFrame',
        CFrameMul(gameIndex(RightArm, 'CFrame'), HandOffset)
    )
    gameNewIndex(
        LeftUpperLeg,
        'CFrame',
        CFrameMul(gameIndex(LeftLeg, 'CFrame'), UpperLegOffset)
    )
    gameNewIndex(
        RightUpperLeg,
        'CFrame',
        CFrameMul(gameIndex(RightLeg, 'CFrame'), UpperLegOffset)
    )
    gameNewIndex(
        LeftLowerLeg,
        'CFrame',
        CFrameMul(gameIndex(LeftLeg, 'CFrame'), LowerLegOffset)
    )
    gameNewIndex(
        RightLowerLeg,
        'CFrame',
        CFrameMul(gameIndex(RightLeg, 'CFrame'), LowerLegOffset)
    )
    gameNewIndex(
        LeftFoot,
        'CFrame',
        CFrameMul(gameIndex(LeftLeg, 'CFrame'), FootOffset)
    )
    gameNewIndex(
        RightFoot,
        'CFrame',
        CFrameMul(gameIndex(RightLeg, 'CFrame'), FootOffset)
    )
    gameNewIndex(
        UpperTorso,
        'CFrame',
        CFrameMul(gameIndex(fakeCharTorso, 'CFrame'), UpperTorsoOffset)
    )
    gameNewIndex(
        LowerTorso,
        'CFrame',
        CFrameMul(gameIndex(fakeCharTorso, 'CFrame'), LowerTorsoOffset)
    )
...
if usedefaultanims then
    task.spawn(function()
        loadstring(
            game:HttpGet(
                'https://raw.githubusercontent.com/somethingsimade/CurrentAngleV2/refs/heads/main/anims'
            )
        )()
    end)
end
